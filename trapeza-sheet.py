...
def main():
    ...
    parser.add_argument("--require-consistency",
                        action="store_true",
                        default=False,
                        help="Require all input sheets to have the same column structure and trigger errors on "
                             "unexpected values. If not specified, the output file will contain the union of all "
                             "headers in all sources, and missing values will be filled with blanks.")
    ...
    parser.add_argument("-f",
                        ...
                        default="csv",
                        help="Specify an output format. If --output is specified, will be inferred from the filename, "
                             "or defaults to CSV.")
    ...
    parser.add_argument("-i",
                        ...
                        default="csv",
                        help="Treat input read from stdin and from files whose type cannot be inferred as being in the "
                             "specified format. Default is CSV.")
    ...
    parser.add_argument("--filter",
                        help="Filter records using the Boolean-valued Python expression provided. "
                             "Each record is provided as a dictionary called 'record'. If specified together with a "
                             "combining operation or --add/--drop, filter is run last.")
    parser.add_argument("--sort",
                        nargs=2,
                        action=SortAction,
                        metavar=("TYPE", "COLUMN"),
                        help="Sort output rows. The first argument should be 'number' or 'string' and controls the "
                             "sort type; the second should be the column name. Multiple --sort and --reverse-sort "
                             "options can be specified to sort on multiple criteria, in order. Sort is run after "
                             "combination operations and filters.")
    parser.add_argument("--reverse-sort",
                        nargs=2,
                        action=SortAction,
                        metavar=("TYPE", "COLUMN"),
                        help="Sort output rows in reverse order. The first argument should be 'number' or 'string' "
                             "and controls the sort type; the second should be the column name. Multiple --sort and "
                             "--reverse-sort options can be specified to sort on multiple criteria, in order. "
                             "Sort is run after combination operations and filters.")
    parser.add_argument("--drop",
                        metavar="COLUMN",
                        help="Drop columns with the name given. May be specified multiple times. "
                             "Drop is run after all combining operations.")
    ...
    parser.add_argument("--primary-key",
                        help="Set the column name where primary record identifiers are stored. If this column is not "
                             "present in all sources, an error will occur. This option is ignored if "
                             "--keep-duplicates is specified.")
    ...
    verbs.add_argument("--union",
                       action="store_true",
                       help="Combine all inputs. If --primary-key is provided, use it to identify duplicates; "
                            "otherwise, use record equality. Earlier rows and earlier sources take precedence "
                            "(unless --keep-duplicates is specified).")
    verbs.add_argument("--intersect",
                       action="store_true",
                       help="Output only rows present in all sources. Identity semantics as in --union. "
                            "Does not retain duplicates.")
    verbs.add_argument("--subtract",
                       action="store_true",
                       help="Subtract, from the first source, any row present in later sources. Identity semantics as "
                            "in --union. Retains duplicate rows in the first source if --keep-duplicates is specified.")
    verbs.add_argument("--xor",
                       action="store_true",
                       help="Output rows present in one and only one source. Identity semantics as in --union. "
                            "Does not retain duplicates")
    ...
    if args.require_consistency:
        if not sources_consistent(sources):
            sys.stderr.write(
                "{}: sources are not consistent and --require-consistency was specified.\n".format(sys.argv[0]))
            return 1
    else:
    ...
    if args.union:
        ...
        if len(sources) > 1:
            sys.stderr.write(
                "{}: more than one source was provided, but no combining operator (--union, --intersect, --subtract, "
                "--xor) was specified.\n".format(sys.argv[0]))
            return 1
        else:
